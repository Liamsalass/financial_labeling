...
# ======================MOBILEBERT INSTALLATION FUNCTIONS======================
...
def return_mobilebert_model(id2label, label2id, quantization_config):
    """
    ...
    try:
        ...
        model = AutoModelForTokenClassification.from_pretrained(
            "google/mobilebert-uncased", num_labels=279, id2label=id2label, label2id=label2id, quantization_config=quantization_config 
        )
    except OSError:
        # If fails to fetch from Hub, look in local rq3/mobilebert-uncased folder
        check_mobilebert_folder()
        model = AutoModelForTokenClassification.from_pretrained(
            "mobilebert-uncased", num_labels=279, id2label=id2label, label2id=label2id, local_files_only=True, quantization_config=quantization_config
        )
